...
data_folder_dir = "C:\Users\Evan Anderson\Desktop\mouse_analysis\";
...
%     [data_folder_dir, folder_info(file_num).name, "\"];
    ...
    trial_sf = table2array(visual_ident_data(:, 3));
    trial_ori = table2array(visual_ident_data(:, 4));

    % Slice the data to only cover the drifting grating portion.
    grating_trials = strcmp(trial_ident, 'dg');
    grating_sf = trial_sf(grating_trials == 1);
    grating_ori = trial_ori(grating_trials == 1);
    grating_stim_timing = stim_timing(grating_trials == 1);   
    num_trials = length(grating_stim_timing);

    % Remove extraneous data to save RAM.
    clear visual_ident_data trial_ident trial_sf trial_ori

    % Compute response vectors.  (Likely is the number of unique orientations
    % and spatial frequencies)
    unique_ori = unique(grating_ori);
    unique_sf = unique(grating_sf);
    num_ori = length(unique_ori);
    num_sf = length(unique_sf);
    ...
    for k = 1:num_cells
        for g = 1:num_trials
            ...
            tuning_responses(g, k) = sum(kept_cells{k} > grating_stim_timing(g) & kept_cells{k} < grating_stim_timing(g) + 0.5 * 20000);
            first_stim = kept_cells{k}(kept_cells{k} > grating_stim_timing(g) & kept_cells{k} <= grating_stim_timing(g) + 10000);
            if ~isempty(first_stim)
            
                stim_diff = first_stim(1) - grating_stim_timing(g);
...
%     final_rast_save{file_num
    ...
    tuning_curve = NaN(num_ori, num_sf, num_cells);
    tuning_curve_z = NaN(num_ori, num_sf, num_cells);
    tuning_curve_std = NaN(num_ori, num_sf, num_cells);

%     cell_array = cell(num_sf, 1);

    for g = 1:num_sf
        test_data = NaN(num_ori, 20, num_cells);
    %     for k = 1:num_cells
    %         temp_array = NaN(num_ori, num_trials, num_cells);
        for j = 1:num_ori
            hello1 = tuning_responses(grating_ori == unique_ori(j) & grating_sf == unique_sf(g), :); % x: trial, y: cell
            hello2 = rast_save(grating_ori == unique_ori(j) & grating_sf == unique_sf(g), :);
            tuning_curve(g,j, :) = mean(tuning_responses(grating_ori == unique_ori(j) & grating_sf == unique_sf(g), :));
            for k = 1:num_cells
...
for sf = 1:length(unique_sf)
    max_values = [];
    cell_num = 1;
    for mouse_num = 1:size(cell_array, 2)
        current_value = cell_array{sf, mouse_num};
...
%             [max_values(cell_num, 1), index] = max(mean_value);
            ...
            if index + 3 >= length(orientations)
    ...
    orientation_array{sf} = max_values;
end
...
for sf = 1:length(unique_sf)
    pulled_data = orientation_array{sf};
    OSI(:, sf) = (pulled_data(:, 1) - pulled_data(:, 3)) ./ (pulled_data(:, 1) + pulled_data(:, 3));
end
...
%         mean_values(, sf) = 
    ...
    sf_seperated_mean_values{sf} = total_mean_value;
end

% Now we need to calculate the data out on a per data basis
for sf = 1:length(unique_sf)
    value_array = sf_seperated_mean_values{sf};
    for cell_num = 1:size(value_array, 2)
        ...
        per_sf_array(cell_num, 1) = R;
        per_sf_array(cell_num, 2) = theta;
    end
    circular_variance{sf} = per_sf_array;
end
...
for sf = 1:length(unique_sf)
    ...
    for mouse_num = 1:size(final_rast_save, 2)
        current_value = final_rast_save{sf, mouse_num};
...
hold on
columns = 2;
rows = length(unique_sf);
plot_num = 1;
for sf = 1:length(unique_sf)
    subplot(rows, columns, plot_num)
    histogram(OSI(find(mouse_ident), sf), 10)
...
for sf = 1:length(unique_sf)
    subplot(rows, columns, plot_num)
    histogram(circular_variance{sf}(find(mouse_ident), 1), 10)
    subplot(rows, columns, plot_num + 1)
    histogram(circular_variance{sf}(find(~mouse_ident), 1), 10)
    plot_num = plot_num + 2;
end
